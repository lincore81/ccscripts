...
-- 2012-2015, written by Dominik 'lincore' Rosehnal. 
...
MAN = [[
arc
A text file archiver for computercraft.
(c)2012, written by Dominik 'lincore'
Rosehnal. You may use, share and modify
this program freely.
]]
...
function pathCrawler(path, files)
    if string.beginsWith(getAbsolutePath(path), ROM_DIR) then
        print("Warning: " .. getAbsolutePath(path) .. " is blacklisted and will be skipped.")
        return
    end
...
-- Used to get rid of a trailing char event that'd otherwise outlast this
-- program and end up on the command line as input.
...
function assertIsExistingWritableDirectory(path)
    ...
    elseif not fs.isReadOnly(path) then
        error("Can not extrack in '" .. path .. "': is read only")
    end
end
